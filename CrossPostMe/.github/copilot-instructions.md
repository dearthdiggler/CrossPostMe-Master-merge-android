...
- open `app/backend/server.py` to inspect mounted routers, DB client lifecycle, and environment var usage (`MONGO_URL`, `DB_NAME`, `CORS_ORIGINS`).
- open `app/backend/routes/*` to copy router patterns (each route file exports `router = APIRouter(...)`).
- open `app/frontend/package.json` and `craco.config.js` for dev/start/build commands and aliases.
...
- `app/backend/routes/ads.py`, `platforms.py`, `ai.py` — router + DB usage examples
- `app/backend/server.py` — app entry, CORS setup, DB client lifecycle
