...
try {
    ...
    if ($idx -ge 0) {
        ...
        $startDouble = $html.LastIndexOf('"', $idx)
        $startSingle = $html.LastIndexOf("'", $idx)
        if ($startDouble -gt $startSingle) { $start = $startDouble } else { $start = $startSingle }
        $endDouble = $html.IndexOf('"', $idx)
        $endSingle = $html.IndexOf("'", $idx)
        if ($endDouble -gt $endSingle -and $endSingle -ne -1) { $end = $endSingle } elseif ($endDouble -ne -1) { $end = $endDouble } else { $end = $endSingle }
